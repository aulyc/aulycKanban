...
export function isMigratableBoardData(data: Record<string, unknown>): boolean {
	return Array.isArray(data['views']) || Boolean(data['work'] && data['personal']) || Array.isArray(data['columns']);
}
...
export function migrateBoardData(raw: unknown): BoardData {
	...
	if (Array.isArray(obj['views'])) {
		...
		if (views.length === 0 || views.every((view) => view.columns.length === 0)) return getDefaultBoardData();
		const rawArchives = obj['archives'] && typeof obj['archives'] === 'object'
