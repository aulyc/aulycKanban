...
- 应用内更新检查：GitHub 优先、Gitee 回退；可手动检查或选择启动后检查，发现新版本后打开官方下载页面
...
- `Check for plugin updates`：检查正式更新
...
- 更新检查默认不自动联网；启用“启动后自动检查”也只读取 `latest.json`。发现新版本后由用户明确点击“打开下载页面”，插件本身不下载 ZIP/provenance、不安装或替换任何插件文件
- 数字签名对当前“检查更新并打开下载页面”流程为 `N/A`，不属于当前待办或发版门禁；若未来新增应用内安装器，再单独建立签名安全门禁
