...
- 聚合入口：顶部固定“全部任务”，象限区固定“全部象限”，可按“任务类型 × 象限”交叉查看
- 左侧任务列表 + 右侧分类导航
...
- 数据持久化：自动保存插件数据
- Markdown 同步：推荐使用一个汇总笔记同步全部任务类型、象限和归档，也可保留按任务类型分笔记的兼容模式
...
- 固定“全部任务”和“全部象限”只改变聚合范围，不覆盖底层保留的具体任务类型和象限选择
- 搜索仅匹配当前“任务范围 × 象限范围”的交集；Enter 提交为搜索标签，`x`、Escape、Backspace 或 Delete 可清除
- 任务区的 `+` 展开一次性输入；在聚合范围新增时，必须显式选择目标任务类型和/或象限
- 删除任务类型会同时删除其中的普通任务、归档任务和同步配置，但不会删除已经生成的 Markdown 文件；至少保留一个任务类型
- 所有任务类型共享同一套象限定义，每个“任务类型 × 象限”的任务内容相互独立
- Tab / Shift+Tab 仍在任务类型、任务内容、象限三个区域间循环；区域内继续使用方向键移动
- 任务列表为当前交叉范围全量渲染（非虚拟列表）
- 同步为“数据变更触发”，纯 UI 切换不触发同步；配置有效路径后会自动创建缺失的目录和 Markdown 文件
- 新安装默认使用“单一汇总笔记”模式；旧数据只要存在已配置的分文件路径，就继续使用“按任务类型分笔记”兼容模式
- 汇总笔记按“任务类型 → 象限”展示进行中与归档任务，并用隐藏的稳定 ID 标记身份；重命名只更新显示标题
- Markdown 是由插件 `data.json` 生成的单向镜像，标记区内的手工修改会在下次同步时被覆盖，标记区外内容保留
