...
采用双源 Policy 1.7.0 的 `obsidian-community` / `obsidian-managed`
...
Policy 1.7.0 新增的 `DMR-011` 只约束 macOS 应用的 `macos-compact` 模式，
对本 Obsidian 插件为 `N/A`，不改变上述 Community 三文件和本地证据分层。
